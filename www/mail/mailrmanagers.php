...
$check = mysql_query("SELECT * FROM rewardemployees WHERE start='$tomorrow'") or die(mysql_error()); 
$verify = mysql_num_rows($check); 

//Check if it is a leap year and send out service date cards
if (date('L')==0){
	if($tomorrow=='02/28'){
		$leapcheck = mysql_query("SELECT * FROM rewardemployees WHERE start='02/29'") or die(mysql_error()); 	
...
$check = mysql_query("SELECT * FROM rewardemployees WHERE birth='$tomorrow'") or die(mysql_error());  
$verify = mysql_num_rows($check); 
